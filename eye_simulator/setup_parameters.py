...
class SetupParameters:
    def __init__(self, hdrmfs_camera_calib, capture_params, eye):
        ...
        self.eyeball_to_cornea_dist = 5.3 / 1000  # m
        self.cornea_radius = 7.8 / 1000  # m
        ...
        # self.translation_error_std = np.array([5.0, 0.0, 0.0])
        # self.rotation_deg_error_std = np.array([0.0, 0.0, 0.0])
        # self.focal_length_error_std = np.array([0.0, 0.0]) * self.M_intr[[0, 1], [0, 1]]
        # self.principal_shift_error_std = np.array([0.0, 0.0]) * self.M_intr[[0, 1], [2, 2]]
    ...
    def randomize_params(self):
        ...
        # Randomize leds positions
        translation_error_ext = np.tile(self.translation_error_std, (self.leds_world_pos.shape[0], 1))
        trans = np.random.normal(0, translation_error_ext) * 0.1
        self.leds_world_pos += trans
        self.leds_world_pos += np.tile(np.random.normal(0, self.translation_error_std), (self.leds_world_pos.shape[0], 1))

        # Randomize M_disp_rsb
        ang_x = np.random.normal(0, self.rotation_deg_error_std[0])
        ang_y = np.random.normal(0, self.rotation_deg_error_std[1])
        ang_z = np.random.normal(0, self.rotation_deg_error_std[2])
        ...
        rot_y = get_rot_y(ang_y)
        rot_z = get_rot_z(ang_z)
        self.M_disp_rsb[0:3, 0:3] = rot_z @ rot_y @ rot_x @ self.M_disp_rsb[0:3, 0:3]

        trans_x = np.random.normal(0, self.translation_error_std[0])
        trans_y = np.random.normal(0, self.translation_error_std[1])
        trans_z = np.random.normal(0, self.translation_error_std[2])
        self.M_disp_rsb[3, 0:3] += np.array([trans_x, trans_y, trans_z])
        ...
        self.eyeball_to_cornea_dist += np.random.normal(0, self.eyeball_to_cornea_dist_error_std)
        self.cornea_radius += np.random.normal(0, self.cornea_radius_error_std)
