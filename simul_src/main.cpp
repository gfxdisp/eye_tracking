...
int main(int argc, char *argv[])
{
    ...
    int max_ims_per_run = 25;

    std::vector<std::thread> threads;

    // Get folder of this executable.
        ...
        threads.emplace_back([i, output_path, path_to_eye, path_to_script, eye, num_images, max_ims_per_run]()
                             {
                                 for (int j = 0; j < num_images; j += max_ims_per_run)
                                 {
                                     ...
                                     std::string command =
                                             "blender " + path_to_eye + " --background --python " + path_to_script +
                                             " -- -e " + eye + " -n " + std::to_string(images_to_render) + " -i " +
                                             std::to_string(i) + " -o " + output_path;
    ...
    // Read CSV files in all created folders and write them to a single CSV file (append to it).
    ...
    for (int i = start_number; i <= end_number; i++)
    {
        ...
        // Skip first line if i > start_number.
        if (i > start_number)
        {
            std::string line;
