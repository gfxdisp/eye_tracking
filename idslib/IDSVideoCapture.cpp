...
IDSVideoCapture::IDSVideoCapture(int index, int AOI_originX, int AOI_originY, float AOI_scale) {
	open(index, AOI_originX, AOI_originY, AOI_scale);
}
...
bool IDSVideoCapture::open(int _index, int AOI_originX, int AOI_originY, float AOI_scale) {
	...
	if (is_InitCamera(&index, NULL) == IS_SUCCESS) {
		...
		m_Width = m_SensorInfo.nMaxWidth * AOI_scale;
		m_Height = m_SensorInfo.nMaxHeight * AOI_scale;
		...
		IS_RECT rectAOI;
		rectAOI.s32X     = AOI_originX;
		rectAOI.s32Y     = AOI_originY;
		rectAOI.s32Width = m_Width;
		rectAOI.s32Height = m_Height;
		INT nRet = is_AOI( m_CamId, IS_AOI_IMAGE_SET_AOI, (void*)&rectAOI, sizeof(rectAOI));

		if(nRet!=0)
			std::cout << "Error defining AOI. Error Code: " << nRet << std::endl;
...
bool IDSVideoCapture::set(int propId, double value) {
	switch (propId) {
		case CAP_PROP_FPS:
			is_SetFrameRate(m_CamId, value, nullptr);
			return true;
		/*case CAP_PROP_FRAME_WIDTH:
...
void IDSVideoCapture::setExposureTime(double exptime) {
	double nexptime;
	is_Exposure(m_CamId,IS_EXPOSURE_CMD_SET_EXPOSURE,&exptime,sizeof(exptime));
	// is_SetExposureTime(m_CamId,exptime, &nexptime);
