...
void PolynomialFit::fit(std::vector<std::vector<float> *> &variables,
    ...
    for (const auto &variable : variables) {
        maximum.push_back(std::abs(*std::max_element(
            variable->begin(), variable->end(), max_abs_func)));
    }
    ...
    for (int i = 0; i < n_coeffs; i++) {
        ...
        coefficients_.push_back(
            (float) (val * scaled_coeffs.at<float>(i, 0) * maximum_output));
    }
}
...
std::vector<std::vector<int>> PolynomialFit::generateMonomials(int order,
    ...
    for (int i = 0; i <= order; i++) {
        ...
        output.insert(output.end(), all_lower_monomials.begin(),
                      all_lower_monomials.end());
    }
    return output;
}
